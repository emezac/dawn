...
def main():
    ...
    if not os.path.exists(upload_file_path):
        try:
            with open(upload_file_path, "w", encoding="utf-8") as f: f.write("Dummy content.")

        except Exception as e: print(f"Warning: Could not create dummy PDF file: {e}")
    ...
    try:
        task1_upload = Task(
            task_id="upload_file_task", name="1. Upload Training Doc", is_llm_task=False,
            tool_name="file_upload", input_data={"file_path": upload_file_path, "purpose": "assistants"},
            parallel=False, next_task_id_on_success="create_vector_store_task"
        )
        task2_create_vs = Task(
            task_id="create_vector_store_task", name="2. Create Vector Store", is_llm_task=False,
            tool_name="vector_store_create", input_data={
                "name": "Training Doc Store Clean",
                "file_ids": ["${upload_file_task.output_data.result}"] # Correct
            },
            parallel=False, next_task_id_on_success="file_search_task"
        )
        task3_file_search = Task(
            task_id="file_search_task", name="3. Extract Doc Insights (RAG)", is_llm_task=False,
            tool_name="file_read", input_data={
                ...
                "max_num_results": 5, "include_search_results": True
            },
            parallel=True,
        ...
        task4_web_search = Task(
            task_id="web_search_task", name="4. Web Search AI Ethics", is_llm_task=False,
            tool_name="web_search", input_data={
                "query": "What are the latest advancements in AI ethics research?",
                "context_size": "medium",
                "user_location": { "type": "approximate", "country": "US", "city": "San Francisco", "region": "CA" }
            },
            parallel=True,
            next_task_id_on_success="generate_summary_task",
        ...
        task3b_regenerate = Task(
            task_id="regenerate_insights_task", name="3B. Regenerate Doc Insights", is_llm_task=True,
            input_data={
            ...
            parallel=False, max_retries=1, next_task_id_on_success="generate_summary_task"
        )
        task4b_default_web = Task(
            task_id="default_web_search_task", name="4B. Default Web Search Result", is_llm_task=True,
            input_data={
        ...
        task5_summarize = Task(
            task_id="generate_summary_task", name="5. Generate Combined Summary", is_llm_task=True,
            input_data={
                "prompt": (
            ...
            parallel=False, max_retries=1, next_task_id_on_success="write_markdown_task"
        )
        task6_write_md = Task(
            task_id="write_markdown_task", name="6. Write Summary to Markdown", is_llm_task=False,
            tool_name="write_markdown", input_data={
    ...
    if not tasks_data: print("No task data available.")
        ...
        if task_id in tasks_data:
            ...
            if output:
                if 'result' in output:
                    ...
                    response_preview = (output['response'][:200] + '...' if len(output['response']) > 200 else output['response'])
                    print(f"  Response (Preview): {response_preview}")
                elif 'error' in output:
                    print(f"  Error: {output['error']}")
                else: print(f"  Output Data: {output}")
            elif task_data.get('status') not in ['pending']: print("  No output data recorded.")
    ...
    if final_md_file_path and os.path.exists(final_md_file_path) and results.get('status') == 'completed':
        ...
        try:
            with open(final_md_file_path, "r", encoding="utf-8") as f: md_content = f.read()
            print(md_content)
            print("----------------------------------------------------")
        except Exception as e: print(f"Error reading final Markdown file: {str(e)}")
    elif results.get('status') == 'completed': print(f"\nWarning: Workflow completed, but final markdown file not found/read.")
    elif results.get('status') == 'failed': print("\nWorkflow failed. Markdown file not generated.")
